...
공유 기능은 송신 API와 별개로, **상대의 공유 시작/종료를 presence로 받아** video·canvas UI를 갱신해야 합니다. 공통 리스너 패턴은 [이벤트 메시지](event.md)를 참고하세요.

| type | 언제 | 앱에서 할 일 예시 |
| ---- | ---- | ----------------- |
| `publish` (`feed.type === 'screen'`) | 그룹에서 상대가 화면 공유를 시작 | `subscribeVideo(id, 'screen')` → screen video DOM에 `srcObject` |
| `screen` | 화면 공유 알림 | UI 배지/레이아웃 전환 (그룹에서 영상 연결은 `publish`가 정석) |
| `subscribed` (`cam: false`) | P2P에서 상대 화면 공유 연결 완료 | `getStream('screen')` → video에 연결 |
| `shareStop` | 상대(또는 본인 쪽 종료 전파) 공유 종료 | screen video/canvas DOM 제거, 레이아웃 원복 |
| `whiteBoard` | 상대 화이트보드 시작 | 캔버스 UI 표시, 필요 시 동기화 |
| `document` | 상대 자료 공유 세션 시작 | 자료 공유 UI 표시 |
| `documentShare` | 상대가 이미지 URL 공유 | `msg.img`로 이미지 렌더 |

{% code title="presence - screen share (group)" %}
```javascript
...
* 관련 타입: [publish](event.md#type-publish), [screen](event.md#type-screen), [subscribed](event.md#type-subscribed), [shareStop](event.md#type-sharestop), [documentShare](event.md#type-documentshare)
