...
<script
  ...
  src="https://knowledgetalk.co.kr:7104/uncompressed/privacyMode.js"
></script>
```
...
- `blurStream`과 `outputCanvas`에 결과물이 표시됩니다.\
...
<table><thead><tr><th width="141">Parameter</th><th width="429">Description</th><th>Example</th></tr></thead><tbody><tr><td>sdk</td><td>Knowledgetalk SDK 인스턴스</td><td>Knowledgetalk</td></tr><tr><td>target</td><td>P2P인 경우 상대방 userId</td><td>"u1234"</td></tr></tbody></table>
...
- Knowledgetalk `init`·방 입장을 완료합니다.
- 원본 `MediaStream`을 `inputVideo.srcObject`에 연결하고 영상이 재생 가능할 때까지 기다립니다.
- 결과를 그릴 `outputCanvas`를 준비합니다.

### 유의사항

- 로컬 미리보기와 원격 송신은 별개입니다. `start()`만으로는 상대 영상이 바뀌지 않습니다.
- **아직 publish하지 않은 경우**: 그룹은 `publishVideo("cam", blurStream)`, P2P는 `publishP2P(userId, "cam", blurStream)`입니다.
- **이미 원본으로 publish한 경우**: connected peer의 sender를 `replaceTrack`합니다. 그룹은 `sendBlurVideoTrack(knowledgetalk)`, P2P는 `sendBlurVideoTrack(knowledgetalk, userId)`입니다.
- P2P에서 새 송신 피어가 생기면 그 피어 연결 완료 후 `sendBlurVideoTrack()`을 다시 호출합니다.
...
// A. 아직 publish하지 않은 경우
await knowledgetalk.publishVideo("cam", blurStream); // 그룹
// await knowledgetalk.publishP2P(userId, "cam", blurStream); // P2P

// B. 이미 원본으로 publish한 경우 (A와 동시에 호출하지 않음)
// await privacyMode.sendBlurVideoTrack(knowledgetalk); // 그룹
// await privacyMode.sendBlurVideoTrack(knowledgetalk, userId); // P2P
