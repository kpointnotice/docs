...
- **아직 publish하지 않은 경우**: 그룹은 `publishVideo("cam", blurStream)`, P2P는 `publishP2P(partnerId, "cam", blurStream)`.
- **이미 원본으로 publish한 경우**: connected peer의 sender를 `replaceTrack`합니다. 그룹은 `sendBlurVideoTrack(knowledgetalk)`, P2P는 `sendBlurVideoTrack(knowledgetalk, partnerId)`.
...
// A. 아직 publish하지 않은 경우 (택1)
await knowledgetalk.publishVideo("cam", blurStream); // 그룹
await knowledgetalk.publishP2P(partnerId, "cam", blurStream); // P2P

// B. 이미 원본으로 publish한 경우 (택1, A와 동시에 호출하지 않음)
await privacyMode.sendBlurVideoTrack(knowledgetalk); // 그룹
await privacyMode.sendBlurVideoTrack(knowledgetalk, partnerId); // P2P
