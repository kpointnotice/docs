...
- `permit` 성공만으로 Guest 화면에 공유·판서 버튼이 생기지는 않습니다. Guest가 상태를 반영한 뒤 해당 API를 호출해야 합니다.
- 권한을 회수할 때도 동일 API에 `false`를 넘기고, Guest UI를 다시 잠급니다.
- 화면 공유 실제 송수신은 [화면 공유 시나리오](share.md#시나리오-화면-공유-송수신)를 참고합니다.
- **텍스트박스(`textbox`)**: SDK `permit`에는 `text` / `textbox` 전용 필드가 없습니다. P2P 서비스에서는 보통 `draw`를 `permit`으로 열고, 텍스트박스 사용 여부는 `inform` 앱 규약으로 별도 전달합니다. Guest는 그 플래그가 true일 때만 `setTool("textbox", ...)` UI를 엽니다.

### Host (권한 부여)

{% code title="permit - host (screen)" %}
...
await knowledgetalk.inform({ type: "permit", screen: true }, guestId);
```

{% endcode %}

{% code title="permit - host (textbox, P2P)" %}

```javascript
// 예: 판서(draw) + 텍스트박스 사용 허용
// textbox 전용 SDK 플래그는 없으므로 draw로 서버 권한을 맞추고,
// text는 inform 앱 규약으로 Guest UI에 전달합니다.
await knowledgetalk.permit(
  guestId,
  undefined, // chat
  true, // draw
...
await knowledgetalk.inform(
  { type: "permit", draw: true, text: true },
  guestId,
);
```
...
knowledgetalk.addEventListener("presence", async (event) => {
  ...
  switch (msg.type) {
    case "inform": {
      ...
      // 앱 상태에 권한 반영 (screen / draw / text 등)
...
function onSelectTextboxTool() {
  if (!getLocalPermit().draw || !getLocalPermit().text) return;
