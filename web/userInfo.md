...
- **`broadcast` (중요)**
  ...
  - 인자 순서상 `target`만 넘기려면 네 번째에 `true`를 넣어야 합니다.  
    `editUserInfo(undefined, false, undefined, userId)`처럼 네 번째에 `userId`를 넣으면 `broadcast`로 해석되어 의도와 달라질 수 있습니다.
...
1. `editUserInfo`로 `video: false`를 설정합니다. (P2P면 `target`에 상대방 `userId`를 넣습니다.)
...
// 다시 켜기
await knowledgetalk.editUserInfo(undefined, true, undefined, true, userId);
...
- “안 보여 주려면 `removePeer` / `removeLocalPeer`로 양쪽에 피어를 끊어야 합니다”라고 오해하지 마세요. 이는 **재연결·세션 종료**용이며 카메라 끄기/가리기와는 다릅니다.
- `editUserInfo(..., broadcast: false)`만으로 상대방 UI까지 바뀔 것이라 기대하지 마세요. **로컬 track만** 바뀌고 방 안 다른 유저에게는 `editUserInfo` presence가 가지 않습니다.
